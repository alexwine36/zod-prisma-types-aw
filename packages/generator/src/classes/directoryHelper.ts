...
export class DirectoryHelper {
	...
	static pathExistsElseCreate(path: string) {
		return (
			DirectoryHelper.pathOrDirExists(path) ||
			Boolean(DirectoryHelper.createDir(path))
		);
	}
	...
	static createDir(path: string, options?: CreateDirOptions) {
		...
		return DirectoryHelper.pathOrDirExists(path) ? path : undefined;
	}
	...
	static removeDir(path?: string | null) {
		...
		if (!DirectoryHelper.pathOrDirExists(path)) return;
		try {
