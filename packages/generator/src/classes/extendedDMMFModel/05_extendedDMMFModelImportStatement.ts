...
export class ExtendedDMMFModelImportStatement extends ExtendedDMMFModelValidatorPattern {
	...
	private _getModelImports() {
		...
		const importList = this._importStatements.flatMap((importStatement) => {
	...
	private _getImports() {
		...
		if (this._automaticImports) {
			this._automaticImports.map((statement) => imports.add(statement));
		}

		if (this.modelImports) {
			this.modelImports.forEach((statement) => {
				imports.add(statement);
			});
		}

		if (this.fieldImports) {
			this.fieldImports.forEach((statement) => {
				imports.add(statement);
			});
		}
	...
	private _getFieldImports() {
		...
		this.fields.forEach((field) => {
			if (field.imports) {
				field.imports.forEach((statement) => {
					imports.add(statement);
				});
			}
		});
