...
import type { ExtendedDMMFDatamodel } from "./extendedDMMFDatamodel";
import type { ExtendedDMMFField } from "./extendedDMMFField";
import type { ExtendedDMMFModel } from "./extendedDMMFModel";
import {
	ExtendedDMMFSchemaArg,
	type ZodValidatorOptions,
	...
	implements DMMF.InputType
{
	...
	private _setImports() {
		...
		const fieldImports = [
			...
			decimalJSImport,
			zodImport,
			...this.fields.flatMap((field) => field.getImports(this.name)),
		];
